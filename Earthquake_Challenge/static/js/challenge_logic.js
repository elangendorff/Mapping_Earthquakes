...
// layers are visible.
L.control.layers(baseMaps, overlays, {collapsed: false, sortLayers: true}).addTo(map);

const quakes7DaysAll = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson"
const quakes7DaysMajor = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson"
const tectonicPlatesSource = "https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json"
...
d3.json(quakes7DaysAll).then(function(data) {
    
    // Moved the `styleInfo`, ``, and `` function definitions to outside the scope
    // of this block so that they can be used in the Major Earthquakes section, too.
...
d3.json(quakes7DaysMajor).then(function(data) {

    // 2.4. Use the same style as the earthquake data.
    // [Defined outside of block]
...
d3.json(tectonicPlatesSource).then(function(data) {
